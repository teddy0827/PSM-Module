...
# TROCS INPUT을 FITTING하고, Residual에서 decorrect
def trocs_decorrect(df_rawdata, df_trocs_input):
    ...
    for (unique_id, diex, diey), group in grouped:
        ...
        trocs_row = df_trocs_input[
            (df_trocs_input['UNIQUE_ID'] == unique_id) &
            (df_trocs_input['dCol'] == diex) &
            (df_trocs_input['dRow'] == diey)
        ]
        ...
        if trocs_row.empty:
            ...
            rk_values = trocs_row.iloc[:, 15:87]
        ...
        residual_x_detrocs = group['residual_x'].values - trocs_fit_x
        residual_y_detrocs = group['residual_y'].values - trocs_fit_y
    ...
    df_trocs_de = df_trocs_de.drop(['UNIQUE_ID', 'TEST', 'DieX', 'DieY'], axis=1)

    # ★★★ concat쓰려면 인덱스 리셋해야함. df_rawdata의 index와 df_psm_de의 index를 기준으로 병합함. 
    df_trocs_de = df_trocs_de.reset_index(drop=True)
