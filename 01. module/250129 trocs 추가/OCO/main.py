...
from calc_regression_oco import (
    kmrc_decorrect,
...
from zernike_analysis_oco import zernike_analysis
...
def main():
    folder_path = 'C:/py_data/nau/782-817/oco'
    ...
    ################################### outlier 판정용 추가 ######################################################################
    # outlier 판정
    df_rawdata = detect_outliers_studentized_residual(df_rawdata, threshold=2.0, group_col='UNIQUE_ID')
    ########################################################################################################
    ...
    # TROCS Decorrect
    df_trocs_input = pd.read_csv('Trocs_Input.csv')
    df_trocs_de = trocs_decorrect(df_rawdata, df_trocs_input)
    df_rawdata = pd.concat([df_rawdata, df_trocs_de], axis=1)
    print(datetime.now().strftime('%Y-%m-%d %H:%M:%S'), 'TROCS Decorrect 완료.')
