...
# Test No 좌표 추출 함수 추가 (UNIQUE_ID 포함)
def extract_test_coord(rawdata_file, rawdata_file_no_header):
    """
    RawData-1 시트에서 Test No, coordinate_X, coordinate_Y, UNIQUE_ID 추출
    (UNIQUE_ID 하나당 Test No가 여러 개 존재하며, 중복 없이 1번만 붙음)
    """
    # 1행 S~U 열(인덱스 18~20), 2행부터 끝까지
    test_coord_df = rawdata_file_no_header.iloc[1:, 18:21].copy()
    test_coord_df.columns = ['Test No', 'coordinate_X', 'coordinate_Y']
    
    # 필수: NaN 제거 + 중복 제거
    test_coord_df = test_coord_df.dropna(subset=['Test No', 'coordinate_X', 'coordinate_Y'])
    test_coord_df = test_coord_df.drop_duplicates(subset=['Test No'])

    # 타입 변환
    test_coord_df['Test No'] = pd.to_numeric(test_coord_df['Test No'], errors='coerce')
    test_coord_df['coordinate_X'] = pd.to_numeric(test_coord_df['coordinate_X'], errors='coerce')
    test_coord_df['coordinate_Y'] = pd.to_numeric(test_coord_df['coordinate_Y'], errors='coerce')
    
    # 메타 정보 추출
    stepseq = rawdata_file.iloc[0, 13]
    lot_id = rawdata_file.columns[13]
    wafer = rawdata_file.iloc[0, 0]
    p_eqpid = rawdata_file.iloc[1, 13]
    photo_ppid = rawdata_file.iloc[11, 13]
    p_time = rawdata_file.iloc[2, 13]
    m_time = rawdata_file.iloc[4, 13]
    mmo_mrc_eqp = rawdata_file.iloc[19, 13]

    # 추가 공정 정보
    step_pitch_x = rawdata_file.iloc[6, 13]
    step_pitch_y = rawdata_file.iloc[7, 13]
    map_shift_x = rawdata_file.iloc[8, 13]
    map_shift_y = rawdata_file.iloc[9, 13]

    # 고유 ID
    unique_id = f"{stepseq}_{p_eqpid}_{photo_ppid}_{mmo_mrc_eqp}_{p_time}_{m_time}_{lot_id}_{wafer}"

    # 동일한 unique_id를 Test No 수만큼 반복
    # test_coord_df['UNIQUE_ID'] = unique_id  

    # 컬럼 정렬: UNIQUE_ID → 좌표 + 공정정보
    test_coord_df.insert(0, 'UNIQUE_ID', unique_id)
    test_coord_df['STEP_PITCH_X'] = step_pitch_x
    test_coord_df['STEP_PITCH_Y'] = step_pitch_y
    test_coord_df['MAP_SHIFT_X'] = map_shift_x
    test_coord_df['MAP_SHIFT_Y'] = map_shift_y
...
def process_nau_file(file_path):
    ...
    return rawdata_df, trocs_input_df, psm_input_df, mrc_df, wk_rk_input_df, test_coord_df


def save_combined_data(rawdata_list, trocs_input_list, psm_input_list, mrc_list, wk_rk_input_list, test_coord_list):
    # 결과 파일을 저장할 폴더 지정
    results_dir = "results"
    ...
    combined_test_coord = pd.concat(test_coord_list, ignore_index=True)
    ...
    combined_test_coord.to_csv(os.path.join(results_dir, "Test_Coord.csv"), index=False)
