...
def detect_outliers_by_residual(df_rawdata, threshold=3.0, group_col='UNIQUE_ID'):
    """
    multi_lot_regression_and_fitting() 수행 후,
    residual_x, residual_y 컬럼을 기준으로 outlier를 판정해주는 함수 예시.

    - group_col 기준으로 그룹핑 (예: UNIQUE_ID)
    - 그룹별 residual_x, residual_y의 표준편차(std) 계산
    - |residual_x| > threshold*std_x 또는 |residual_y| > threshold*std_y면 이상치 표시

    Args:
        df_rawdata: 잔차(residual_x, residual_y)가 포함된 DataFrame
        threshold: 몇 배의 표준편차에서 컷할지 (default=3)
        group_col: 그룹핑할 컬럼명 (기본값 'UNIQUE_ID')

    Returns:
        outlier 컬럼이 추가된 df_rawdata (inplace로 변경 + 동일 객체 return)
    """
    ...
    for g_id, group in grouped:
        # 그룹별 residual_x, residual_y 추출
        res_x = group['residual_x']
        res_y = group['residual_y']

        # 표준편차 (편차가 0인 경우 대비)
        std_x = res_x.std(ddof=1) if len(res_x) > 1 else 0
        std_y = res_y.std(ddof=1) if len(res_y) > 1 else 0
        ...
        # 이상치 조건
        cond_x = res_x.abs() > (threshold * std_x)
        cond_y = res_y.abs() > (threshold * std_y)
        # 둘 중 하나라도 True면 outlier로 간주 (OR)
        outlier_mask = cond_x | cond_y

        # df_rawdata에 outlier 표시 적용
